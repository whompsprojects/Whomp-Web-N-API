...
$(document).ready(function () {
    try {
        if (!Convert.objectToBool(WebIsAdd)) {
            ...
            $(document).on('click', '.clsWebMenu', function () { SetWeb.Scroller(this); });
...
var SetWeb = {
    ...
    Menu: function () {
        try {
            ...
            $(menuDet).each(function (k, v) {
                ...
                if (v.ParentID == 0) {
                    arrMenu.push(v.Href);
                    mainMenu += '<li isUsed="1" ' + active + '><a href=' + v.Href + ' class="clsWebMenu">' + v.Menu_Name + '</a></li>';
                }
                else {
                    if (subMenu == '') {
                    ...
                    subMenu += '<li subLi="1" isUsed="1"><a href="' + v.Href + '">' + v.Menu_Name + '</a></li>';
                }
            });
            if (subMenu != '')
                subMenu += '</ul></li>';
            ...
            if (!Convert.objectToBool(VB_WebIsView)) {
                VrVw.html += '<li isUsed="0"><a class="cls_EditWeb" id="a_Edit_Menu"><i class="fa fa-edit"></i></a></li>';
            }
    ...
    Lead_Fields: function () {
        try {
            ...
            $(Lead_FieldsDet).each(function (k, v) {
                ...
                if (i == 2 && !Convert.objectToBool(VB_WebIsView))
    ...
    Products: function () {
        try {
            ...
            $(Products_Det).each(function (k, v) {
                ...
                if ((i == (lst1 + 1)) && (!Convert.objectToBool(VB_WebIsView)))
    ...
    Team: function () {
        try {
            ...
            $('.cls_Department').html('');
            VrVw.html = '';
            var i = 1, totalDep = 0, totalLen = 0, cls_all ='cbp-filter-item-active ';


            $(Team_Department_Det).each(function (k, v) {
                if (k == 0) {
                    totalDep = Team_Department_Det.length - 1;
                    cls_all = 'cbp-filter-item-active';
                }
                else {
                    var tmpobj = $.grep(Team_Det, function (gv) {
                        if (gv.Team_Department_Code == v.Department_Code) {
                            return gv;
                        }
                    });
                    totalDep = tmpobj.length;
                    cls_all = '';
                }
                i++;
                VrVw.html = '<div id="div_Department_' + i + '" data-filter=".' + v.Department_Name + '" class="cbp-filter-item ' + cls_all+'" val="' + v.Department_Name + '" code="' + v.Department_Code + '">'
                    + '' + v.Department_Name + ' (<div class="cbp-filter-counter">' + totalDep+ '</div>)'
                    + '</div>';
    ...
    SocailNtwk: function () {
        try {
            ...
            $(SocailNtwk_Det).each(function (k, v) {
                i++;
                socialNtwkHref = v.Social_Ntwk_Href;
    ...
    CallGenerateLead: function () {
        try {
            ...
            $('#btn_SubmitLead').prop('disabled', true);
            WMP_Common.ShowProcess();
            ...
            console.log(ex);
            WMP_Common.SlideErr();
            $('#btn_SubmitLead').prop('disabled', false);
        }
    },
