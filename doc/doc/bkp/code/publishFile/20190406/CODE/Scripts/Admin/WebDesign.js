...
var Team = {
    ...
    IntializeTeam: function () {
        ...
        $(document).on('change', '[id^=file_Team]', function () { Team.SetTeamImg(event, this) });
        ...
        try {
            ...
            $('[id^=a_TeamName]').each(function (k, v) {
                ...
                Var_SC.html = '<div class="col-xs-12 row">'
                    ...
                    + '</div>'
                    + '<div class="col-xs-4">'
                    + '<input type="text" id="ta_TeamDesc_' + i + '" value="" class="form-control" aria-describedby="basic-addon2" placeholder="Enter team description" title="Enter team description"/>'
                    + '</div>'
                    + '<div class="col-xs-2">'
...
var FeedBk = {
    ...
    IntializeFeedBk: function () {
        ...
        $(document).on('click', '[id=a_Edit_FeedBk_BkGrnd_Img]', function () { $('#file_FeedBk_BkGrnd_Img').trigger('click'); });
        $(document).on('change', '[id=file_FeedBk_BkGrnd_Img]', function () { WMP_Common.SetImg_TR(event, this, 'img_FeedBk_BkGrnd_Img', 2000); });
    },
    ...
    PopUp_FeedBk: function () {
        ...
        $('[id^=a_FeedBk_Name]').each(function (k, v) {
            ...
            Var_SC.html = '<div class="col-xs-12 row">'
                ...
                + ' </div>'
                + '<div class="col-xs-2">'
                + '<textarea id="ta_FeedBk_Desc_' + i + '"  value="' + FeedBk_Desc + '" class="form-control" aria-describedby="basic-addon2" placeholder="Enter feedback description" title="Enter feedback description"></textarea>'
                + '</div>'
                + '<div class="col-xs-2">'
    ...
    Set_FeedBk: function () {
        ...
        $('[id^=txt_FeedBk_Name]').each(function (k, v) {
            ...
            FeedBk_Desc = Convert.objectToString($(this).closest('.row').find('[id^=ta_FeedBk_Desc]').attr('value'), '');
...
var Packs = {
    ...
    PopUp_Packs: function () {
        ...
        $('[id^=h2_Package_Name]').each(function (k, v) {
            ...
            Var_SC.html = '<div class="col-xs-12 row">'
                ...
                + '</div>'
                + '<div class="col-xs-2">'
                + ' <input type="text" id="txt_Package_Price_' + i + '" value="' + Pack_Price + '" class="form-control allownumericwithoutdecimal" aria-describedby="basic-addon2" placeholder="Enter package price" title="Enter package price" />'
                + ' </div>'
                + '<div class="col-xs-2">'
                + '<input type="text" id="txt_Package_Price_Per_' + i + '" value="' + Pack_Price_Per + '"  class="form-control" aria-describedby="basic-addon2" placeholder="Enter price per" title="Enter price per" />'
                + ' </div>'
                + '<div class="col-xs-1">'
    ...
    Set_Packs: function () {
        ...
        $('[id^=txt_Package_Name]').each(function (k, v) {
            ...
            i++;
            Pack_Price = Convert.objectToString($(this).closest('.row').find('[id^=txt_Package_Price]').val(), '');
            Pack_Price_Per = Convert.objectToString($(this).closest('.row').find('[id^=txt_Package_Price_Per]').val(), '');
    ...
    Add_Packs: function (currThis) {
        ...
        Pack_Price = Convert.objectToString($(currThis).closest('.row').find('[id^=txt_Package_Price]').val(), '');
        Pack_Price_Per = Convert.objectToString($(currThis).closest('.row').find('[id^=txt_Package_Price_Per]').val(), '');
        ...
        if (Convert.IsObjectNullOrEmpty(Pack_Name) || Convert.IsObjectNullOrEmpty(Pack_Price) || Convert.IsObjectNullOrEmpty(Pack_Price_Per) || Convert.IsObjectNullOrEmpty(Pack_Point) || Convert.IsObjectNullOrEmpty(Pack_BTN_HDR_Name)) {
            $('#div_Error').html(WMP_Common.ErrorPopUp('Please enter the value'));
            $('#div_Error').modal('show');
            return false;
        }
    ...
    PopUp_PacksPoints: function (currThis) {
        ...
        Var_SC.html = '';
        var i = 0, arrPackPoint = [], arrIsPackPointChk = [];

        Pack_Point = Convert.objectToString($(currThis).closest('div').find('[id^=hdn_Pack_Point]').val(), '');
    ...
    Set_PacksPoint: function (currThis) {
        ...
        $('[id^=txt_PackagePoints]').each(function (k, v) {
            ...
            Is_Pack_Point_Chk = IsPackPointChk + (Convert.objectToBool($(this).closest('.row').find('[id^=chk_IsPackPointsChk]').prop('checked'), 0) ? '1' : 0) + '^';
        });
    ...
    setComp: function () {
        try {
            if (!Convert.objectToBool(WebIsAdd)) {
                ...
                $('#strg_CreateComp').html(ComDet.Company_Name);
                $('#txt_Company_Name').val(ComDet.Company_Name);
                $('#txt_Contact_Person').val(ComDet.Contact_Person);
                $('#txt_Company_Email').val(ComDet.Company_Email);

                $('#txt_Mob_1').val(ComDet.Mob_1);
                $('#txt_Tel_1').val(ComDet.Tel_1);
                $('#txt_Aadhar_Card_NO').val(ComDet.Aadhar_Card_NO);
                $('#txt_Pan_Card_No').val(ComDet.Pan_Card_No);

                $('#txt_Company_Specialist_In').val(ComDet.Company_Specialist_In);
                $('#txt_Company_Address').val(ComDet.Company_Address);
            }
        }
        catch (ex) {
